...
data {
  ...
  // NEW: missing HTS indices
  int<lower=0> n_miss;
  int<lower=1, upper=niter> miss_indices[n_miss];
}
...
// mild penalty for missing years   
    for (k in 1:n_miss) {
    int i = miss_indices[k];
    if (hts_m[i] > 1e4) {
      target += -0.5 * square((hts_m[i] - 1e4) / 2e3);
    }
  }
}
...
# male mortality rate (with age group)
age_grp_m <- list("15-24" = 16:25, "25-34" = 26:35, "35-49" = 36:50, "50+" = 51:101)
mx_male_mdg <- mxM1[mxM1$name == "Madagascar", c("name", "age", as.character(2010:2024))] # wpp mortality rates
...
mort_mat_m <- t(sapply(seq(start, end), function(current_yr) 
  calc_mort_agegrp(current_yr, wpp_m, mx_male_mdg, age_grp_m)))
dimnames(mort_mat_m) <- list(seq(start, end), names(age_grp_m))
mort_mat_m <- mort_mat_m[1:13, ] # 2024 not needed as model stops here

# female mortality rate (with age group)
age_grp_f <- list("15-24" = 16:25, "25-34" = 26:35, "35-49" = 36:50, "50+" = 51:101)
mx_female_mdg <- mxF1[mxF1$name == "Madagascar", c("name", "age", as.character(2010:2024))]
...
mort_mat_f <- t(sapply(seq(start, end), function(current_yr) 
  calc_mort_agegrp_f(current_yr, wpp_f, mx_female_mdg, age_grp_f)))
dimnames(mort_mat_f) <- list(seq(start, end), names(age_grp_f))
mort_mat_f <- mort_mat_f[1:13, ] # 2024 not needed as model stops here
...
rownames(agegrp_deno_f) <- c("agegrp1", "agegrp2", "agegrp3", "agegrp4")
colnames(agegrp_deno_f) <- c("2018", "2021")
...
rownames(agegrp_num_f) <- c("agegrp1", "agegrp2", "agegrp3", "agegrp4")
colnames(agegrp_num_f) <- c("2018", "2021")
...
rownames(agegrp_deno_m) <- c("agegrp1", "agegrp2", "agegrp3", "agegrp4")
colnames(agegrp_deno_m) <- c("2018", "2021")
...
rownames(agegrp_num_m) <- c("agegrp1", "agegrp2", "agegrp3", "agegrp4")
colnames(agegrp_num_m) <- c("2018", "2021")
...
# plot for madagascar ( sex and age stratified)
options(scipen = 999)
par(mfrow = c(1, 3), oma = c(0, 0, 0, 0), mar = c(4, 4, 3, 1))
# survey fit
col_age_m <- c("steelblue4", "turquoise3", "seagreen1", "goldenrod3")
...
#  plot men 
plot(svy_m[[1]]$`50%` ~ time, type = "l", col = col_age_m[1], 
     lwd = 3, ylab = "Ever used HIVST(%)", ylim = c(0, 0.1),
     main = "Men")
for (a in 1:4) {
  polygon(x = c(time, rev(time)),
          y = c(svy_m[[a]]$`2.5%`, rev(svy_m[[a]]$`97.5%`)),
...
polygon(x = c(time, rev(time)),
        ...
        col = yarrr::transparent("cyan4", trans.val = 0.5), border = NA)
points(hts_dat ~ I(yr_hts + 0.5), pch = 16, col = "goldenrod3", cex = 1.25)
mtext("Madagascar", outer = TRUE, side = 3, line = 1, cex = 1.5)
...
wpp_m <- popM1[popM1$name == "Eswatini", !(colnames(popM1) %in% as.character(c(1949:2009)))]
wpp_f <- popF1[popF1$name == "Eswatini", !(colnames(popF1) %in% as.character(c(1949:2009)))]
...
# male mortality rate (with age group)
age_grp_m <- list("15-24" = 16:25, "25-34" = 26:35, "35-49" = 36:50, "50+" = 51:101)
mx_male_Eswatini <- mxM1[mxM1$name == "Eswatini", c("name", "age", as.character(2010:2024))] # wpp mortality rates
...
mort_mat_m <- t(sapply(seq(start, end), function(current_yr) 
  calc_mort_agegrp(current_yr, wpp_m, mx_male_Eswatini, age_grp_m)))
dimnames(mort_mat_m) <- list(seq(start, end), names(age_grp_m))
mort_mat_m <- mort_mat_m[1:13, ] # 2024 not needed as model stops here

# female mortality rate (with age group)
age_grp_f <- list("15-24" = 16:25, "25-34" = 26:35, "35-49" = 36:50, "50+" = 51:101)
mx_female_Eswatini <- mxF1[mxF1$name == "Eswatini", c("name", "age", as.character(2010:2024))]
...
mort_mat_f <- t(sapply(seq(start, end), function(current_yr) 
  calc_mort_agegrp_f(current_yr, wpp_f, mx_female_Eswatini, age_grp_f)))
dimnames(mort_mat_f) <- list(seq(start, end), names(age_grp_f))
mort_mat_f <- mort_mat_f[1:13, ] # 2024 not needed as model stops here
...
// mild penalty for missing years   
    ...
    int i = miss_indices[k];
    if (hts_m[i] > 767960) {
      target += -0.5 * square((hts_m[i] - 767960) / 2e4);
    }
  }
}
...
# female survey denominators
agegrp_deno_f <- matrix(
  c(642, 484, 479, -999,    # 2021 deno female
    1552, 1348, 1612, 1076),    # 2022 deno female
  nrow = 4,                     #  age grp
  ncol = 2,                     # survey years
  byrow = FALSE                 
)
rownames(agegrp_deno_f) <- c("agegrp1", "agegrp2", "agegrp3", "agegrp4")
colnames(agegrp_deno_f) <- c("2021", "2022")
...
rownames(agegrp_num_f) <- c("agegrp1", "agegrp2", "agegrp3", "agegrp4")
colnames(agegrp_num_f) <- c("2021", "2022")

# male survey denominators
agegrp_deno_m <- matrix(
  c(601, 377, 349, -999,    # 2021 
    1425, 924, 1110, 587),      # 2022 
  nrow = 4,                     #  age grp
  ncol = 2,                     # survey years
  byrow = FALSE                 
)
rownames(agegrp_deno_m) <- c("agegrp1", "agegrp2", "agegrp3", "agegrp4")
colnames(agegrp_deno_m) <- c("2021", "2022")
...
rownames(agegrp_num_m) <- c("agegrp1", "agegrp2", "agegrp3", "agegrp4")
colnames(agegrp_num_m) <- c("2021", "2022")
...
# we plot men first
plot(svy_m[[1]]$`50%` ~ time, type = "l", col = col_age_m[1], 
     lwd = 3, ylab = "Ever used HIVST(%)", ylim = c(0, 0.5),
     main = "Men")
for (a in 1:4) {
  polygon(x = c(time, rev(time)),
          y = c(svy_m[[a]]$`2.5%`, rev(svy_m[[a]]$`97.5%`)),
...
polygon(x = c(time, rev(time)),
        ...
        col = yarrr::transparent("cyan4", trans.val = 0.5), border = NA)
points(hts_dat ~ I(yr_hts + 0.5), pch = 16, col = "goldenrod3", cex = 1.25)
mtext("Eswatini", outer = TRUE, side = 3, line = 1, cex = 1.5)
