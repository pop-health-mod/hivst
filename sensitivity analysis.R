...
saveRDS(fit, file = "hivst_stan_fit_sdrw1.rds") 
fit <- readRDS("hivst_stan_fit_sdrw1.rds") 

#-------------- Change 2: Doubing the constraint (40× 0.10% coverage or 8× historical maximum)---------
# the model will be saved as "hivst_stan_fit_increased_threshold.rds"
...
for (c in seq_len(n_cnt)) {
  ...
  if (prop < 0.1) {                             # if < 0.10 % of pop, 0.1 % × pop × 40
    thr_c <- 0.001 * pop_c * 40                  
  } else {                                      # if ≥ 0.10 %, old rule
    thr_c <- 8 * max_hts                       
  }
...
max_n_miss <- max( lengths(missing_idx_list) )  
missing_i_mat <- matrix(1L, nrow = n_cnt, ncol = max_n_miss)  
n_miss <- integer(n_cnt)
