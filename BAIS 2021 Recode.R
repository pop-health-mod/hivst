...
# Weighted prop
bais_design <- svydesign(ids = ~psu, strata = ~strata, weights = ~ind_wt, data = bais, nest = TRUE)
...
#hivst_by_sex <- svyby(~I(hivst_use == 1), ~sex, bais_design, svyciprop, method = "logit", vartype = "ci")

#sex I(hivst_use == 1)       ci_l       ci_u
# 0        0.02348760 0.01860713 0.02960954
