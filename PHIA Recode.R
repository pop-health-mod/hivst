...
#Note: While merging, some observations were not matched because they weren't in the biomarker dataset.
...
#kenphia_design <- svydesign(ids = ~psu, strata = ~strata, weights = ~ind_wt, data = kenphia2018, nest = TRUE)

# Calculate proportion by sex (f: 3.8, m: 4.5)
#hivst_by_sex <- svyby(~I(hivst_use == 1), ~sex, kenphia_design, svyciprop, method = "logit", vartype = "ci")
...
# Saving
saveRDS(list_phia, file = "list_phia_dataframe.rds")

#--------------------------------

#manually checking eswatini hivst use
#nrow(swzphia2021)
#table(swzphia2021$hivst_use)
#2312/12043 = 0.19 (unweighted)
#ss <- svydesign(ids = ~psu, strata = ~strata, weights = ~ind_wt, data = swzphia2021, nest = TRUE)
#prop <- svyciprop(~I(hivst_use == 1), design = ss, method = "logit", level = 0.95)
# 0.20 (.188, 0.213)


#weighted hivst_use for Malawi (unweighted 0.23, weighted 0.22)
mphia <- svydesign(ids = ~psu, strata = ~strata, weights = ~ind_wt, data = mwiphia2020, nest = TRUE)
useprop <- svyciprop(~I(hivst_use == 1), design = mphia, method = "logit", level = 0.95)
# 0.0704 (0.0647 0.0766)
hivst_by_sex_mwiphia <- svyby(~I(hivst_use == 1), ~sex, mphia, svyciprop, method = "logit", vartype = "ci")
#sex I(hivst_use == 1)       ci_l       ci_u
#0   0        0.06306585 0.05711917 0.06958596
#1   1        0.07856546 0.07149646 0.08626846

#knwprop <- svyciprop(~I(hivst_knwldge == 1), design = mphia, method = "logit", level = 0.95)
# 0.317(0.304, 0.329)

#effective ss calc
#male
mphia_m <- subset(mphia, sex==1)
useprop_m <- svyciprop(~I(hivst_use == 1), design = mphia_m, method = "logit", level = 0.95)
se_hivst_use_m <- SE(useprop_m)
mwiprop_hivst_use_m <- coef(useprop_m)[1]  
eff_ss_m <- (mwiprop_hivst_use_m * (1 - mwiprop_hivst_use_m)) / (se_hivst_use_m^2)
numerator_m <- mwiprop_hivst_use_m * eff_ss_m

#female
mphia_f <- subset(mphia, sex==0)
useprop_f <- svyciprop(~I(hivst_use == 1), design = mphia_f, method = "logit", level = 0.95)
mwiprop_hivst_use_f <- coef(useprop_f)[1]  
se_hivst_use_f <- SE(useprop_f)
eff_ss_f <- (mwiprop_hivst_use_f * (1 - mwiprop_hivst_use_f)) / (se_hivst_use_f^2)
numerator_m <- mwiprop_hivst_use_f * eff_ss_f

#Male: Den: 5164.086, Num: 405.7188 
#Female: Den: 5920.471, Num: 373.3795


# Create separate dataframes for male and female for each PHIA survey
...
# List for female datasets
list_phia_f <- list(namphia2017f, kenphia2018f, lsophia2020f, zwephia2020f, mwiphia2020f, mozphia2021f, swzphia2021f)

# List for male datasets
list_phia_m <- list(namphia2017m, kenphia2018m, lsophia2020m, zwephia2020m, mwiphia2020m, mozphia2021m, swzphia2021m)
